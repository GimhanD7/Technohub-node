...
export default function AdminCourseManagement() {
  ...
  const loadTeachers = async () => {
    setIsLoading(true);
    const data = await fetchApi("/course/get_teachers");
  ...
  const handleBackToCourses = () => {
    setSelectedCourse(null);
    setModules([]);
    loadCourses(selectedTeacher.id);
  };
  ...
  const handleImageUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        const compressedBase64 = canvas.toDataURL("image/webp", 0.6); // aggressively compress to WebP
  ...
  const handleSaveCourse = async (e) => {
    ...
    const body = editCourseId 
      ...
      : { teacher_id: selectedTeacher.id, ...courseForm };
  ...
  const handleDeleteCourse = (courseId) => {
    showConfirm("Delete Course", "Are you sure? This will delete all modules and materials inside it.", "error", async () => {
      ...
      if (data.success) loadCourses(selectedTeacher.id);
      else showAlert("Error", data.message);
    });
  };
  ...
  if (isLoading && !selectedTeacher && teachers.length === 0) {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 relative pb-12">
            ...
            {teachers.map(t => (
              ...
              >
                {t.profile_picture ? (
                  ...
                  <div className="w-12 h-12 rounded-full bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold text-xl group-hover:bg-blue-600 group-hover:text-white transition-colors shrink-0">
      ...
      {/* Stage 2: Manage Courses for Teacher */}
      {selectedTeacher && !selectedCourse && (
        <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <button onClick={handleBackToTeachers} className="p-2 bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-slate-800 rounded hover:bg-gray-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50 text-gray-500 dark:text-white shadow-sm transition-colors">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {displayedCourses.length === 0 && !isLoading ? (
              <p className="text-gray-500 dark:text-white col-span-3 p-8 text-center bg-gray-50 dark:bg-slate-800/50 border border-dashed rounded-lg">No courses found matching your criteria.</p>
            ) : (
      ...
      {/* Stage 3: Manage Course Sub-Categories & Contents */}
      {selectedCourse && (
        <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[12px] text-slate-600 dark:text-white font-medium">{selectedTeacher?.full_name}</span>
                </div>
              ...
              <div key={mod.id} className="bg-white dark:bg-[#1e293b] rounded-lg border border-gray-200 dark:border-slate-800 shadow-sm overflow-hidden">
                <div className="px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-800 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button onClick={() => openModuleModal(mod)} className="p-1.5 text-blue-500 dark:text-blue-400 hover:text-blue-600 dark:hover:text-blue-300 bg-blue-50 dark:bg-blue-900/20 rounded transition-colors" title="Edit Module">
                      <Edit3 className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => handleDeleteModule(mod.id)} className="p-1.5 text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 bg-red-50 dark:bg-red-900/20 rounded transition-colors" title="Delete Module">
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  </div>
                </div>
                      ...
                      {materials[mod.id].map(mat => (
                        <div key={mat.id} className="flex items-center justify-between p-3 rounded bg-gray-50 dark:bg-slate-800/50 border border-gray-100 dark:border-slate-800/50 hover:border-blue-100 dark:border-blue-900/50 hover:bg-blue-50 dark:hover:bg-blue-900/20/30 transition-colors group">
                          ...
                          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button onClick={() => openMaterialModal(mod.id, mat)} className="p-1.5 text-blue-400 hover:text-blue-600 dark:hover:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/30 rounded transition-colors"><Edit3 className="w-3.5 h-3.5" /></button>
                            <button onClick={() => handleDeleteMaterial(mat.id)} className="p-1.5 text-red-400 hover:text-red-600 dark:hover:text-red-300 hover:bg-red-100 dark:hover:bg-red-900/30 rounded transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          <div className="bg-white dark:bg-[#1e293b] rounded-xl shadow-2xl w-full max-w-md border border-gray-200 dark:border-slate-800">
            ...
            <form onSubmit={handleSaveMaterial} className="p-6 space-y-4">
              ...
              {materialForm.type === 'pdf' && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    }} className={`p-2.5 border rounded flex items-center justify-center gap-2 text-[12px] font-semibold ${materialSource === 'upload' ? 'border-blue-500 bg-blue-50 text-blue-700 dark:bg-blue-900/20' : 'border-gray-200 dark:border-slate-800 text-gray-500 dark:text-white'}`}>
                    ...
                    <button type="button" onClick={() => {
                      ...
                      if (materialForm.content_url?.startsWith('/uploads/')) setMaterialForm({...materialForm, content_url: ''});
                      setMaterialUploadName('');
                    }} className={`p-2.5 border rounded flex items-center justify-center gap-2 text-[12px] font-semibold ${materialSource === 'link' ? 'border-blue-500 bg-blue-50 text-blue-700 dark:bg-blue-900/20' : 'border-gray-200 dark:border-slate-800 text-gray-500 dark:text-white'}`}>
              ...
              {materialForm.type === 'pdf' && materialSource === 'upload' ? (
                ...
                <div>
                  <label className="block text-[11px] font-bold text-gray-500 dark:text-white uppercase tracking-wider mb-1.5">{materialForm.type === 'video' ? 'YouTube, Google Drive, or Video Link *' : 'Resource URL (Link) *'}</label>
                  <input required type="url" value={materialForm.content_url ?? ''} onChange={e => setMaterialForm({...materialForm, content_url: e.target.value})} className="w-full p-2.5 text-[13px] border border-gray-200 dark:border-slate-800 rounded focus:ring-1 focus:ring-blue-500 focus:outline-none" placeholder={materialForm.type === 'video' ? 'https://drive.google.com/file/d/.../view or YouTube link' : 'https://...'} />
                  {materialForm.type === 'video' && (
                    <p className="mt-2 text-[11px] text-gray-500 dark:text-white">For Google Drive videos, set sharing to Anyone with the link can view.</p>
                  )}
                </div>
              )}
              <div>
